...
void logMemoryStatus(const char* message) {
    ...
    cudaCheck(cudaDeviceGetGraphMemAttribute(0, cudaGraphMemAttrUsedMemCurrent,
                                             &usedMemCurrent));
    cudaCheck(
        cudaDeviceGetGraphMemAttribute(0, cudaGraphMemAttrUsedMemHigh, &usedMemHigh));
    cudaCheck(cudaDeviceGetGraphMemAttribute(0, cudaGraphMemAttrReservedMemCurrent,
                                             &reservedMemCurrent));
    cudaCheck(cudaDeviceGetGraphMemAttribute(0, cudaGraphMemAttrReservedMemHigh,
                                             &reservedMemHigh));

    // Print basic memory info
    std::cout << message << " - Free Memory: " << free_gb
              << " GB, Total Memory: " << total_gb
              << " GB, Graph Memory Usage: " << usedMemCurrent / (double)(1 << 30)
              << " GB, Graph Reserved Memory: " << reservedMemCurrent / (double)(1 << 30)
              << " GB\n";
}
